...
export class GeminiService {
  ...
  constructor() {
    // FIX: Throw an error during construction if the API key is missing.
    // This is a fail-fast approach for critical configuration errors.
    if (!this.apiKey) {
      throw new Error("API_KEY environment variable not set. The application cannot function without it.");
    }
    this.ai = new GoogleGenAI({ apiKey: this.apiKey });
  }

  async getInterpretation(text: string, value: number): Promise<string> {
    // FIX: The method now throws on error for consistent error handling, letting the caller manage UI feedback.
    ...
    try {
      ...
      // FIX: Throw an error instead of returning a string for consistency.
      throw new Error('Failed to get interpretation from Gemini.');
    }
  }

  async getAstrologicalReading(day: number, month: number, year: number, gender: 'زن' | 'مرد'): Promise<string> {
    // FIX: The method now throws on error for consistent error handling.
    ...
    const prompt = `
      ...
      تحلیل خود را به صورت یک روایت منسجم، شیوا و قدرمند به زبان فارسی ارائه دهید. از لیست کردن اطلاعات بپرهیزید. تحلیل باید این عناصر را در هم بیامیزد:
    ...
    try {
      ...
      // FIX: Throw an error instead of returning a string for consistency.
      throw new Error('Failed to get astrological reading from Gemini.');
    }
  }

  async getMostahsalehAnalysis(userName: string, motherName: string): Promise<MostahsalehResult> {
    // FIX: The API key check is removed as the constructor now guarantees its presence.
  ...
  async getSigilInterpretation(intent: string): Promise<string> {
    ...
    try {
      ...
      // FIX: Throw an error instead of returning a string for consistency.
      throw new Error('Failed to get sigil interpretation from Gemini.');
    }
  }
